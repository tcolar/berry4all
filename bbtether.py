...
MODEM_ACK2 = [0x00, 0x00, 0x00, 0x00, 0xb0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc2, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x78, 0x56, 0x34, 0x12]
MODEM_ACK3 = [0x4, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x1c, 0x0, 0x0, 0x0, 0x78, 0x56, 0x34, 0x12 ]
MODEM_ACK4 = [0x0, 0x0, 0x0, 0x0, 0x20, 0x0, 0x0, 0x0, 0x20, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,  0x18, 0x0, 0x0, 0x0, 0x78, 0x56, 0x34, 0x12 ]
...
class BBModem:		
	...
	def start(self):
		...
		print "\nModem pty: ",os.ttyname(master)," - ",os.ttyname(slave)
		...
		#writeThread=BBModemWriteThread(self.handle,self.readpt,slave)
		#writeThread.start()	
		
		bbThread=BBModemThread(self.handle,self.readpt,self.writept,master,slave)
		bbThread.start()
...
class BBModemThread( threading.Thread ):
	...
	def __init__(self, hnd, read, write, master, slave):
		...
		while( not self.done):
			try:
				...
				try:
					...
					if(len(bytes)>0):
						if(same_tuple(bytes,MODEM_ACK3) or same_tuple(bytes,MODEM_ACK4)):
							print "Received ACK"
						else:
